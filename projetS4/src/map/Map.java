...
public class Map{
	 ...
	 * @param chemin Chemin du fichier o� se trouve la map
	 */
	public Map(int numMap, String chemin){
		if(!Files.exists(Paths.get(chemin)))
			throw new IllegalArgumentException("Fichier non trouv�");
	...
	public int nbLigne(String map){
		...
		Scanner sc = scanner.useDelimiter("\n"); //en deux �tapes pour pouvoir fermer scanner
		String ligne ="";
		int i = 0;
		if(sc.hasNext()){
			ligne = sc.next();
